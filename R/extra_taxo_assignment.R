...
#'
#' @description
#' This function takes in a pyloseq object with taxonomy data or a dataframe containing taxonomy information and currently adds the protist and
...
#' (using the taxo_normalisation function of this package for example) where protists and Archaeplastida (excl. Viridiplantae) taxa
...
extra_taxo_assignment = function(obj, sqlFile){
  ...
  # Step 2: Get all descendant taxa for protist taxa
  ...
  update_kingdom <- function(tax_row) {
    # Check if Kingdom is NA or an empty string, and if any of the higher ranks belong to protist
